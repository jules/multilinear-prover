...
where
    ExtensionField<F, 2>: std::fmt::Debug + std::fmt::Display,
{
...
impl<F: Field, const N: usize> FieldExtension<F> for ExtensionField<F, N> {
    ...
    fn from_coeffs_in_base_ref(coeffs: &[&F]) -> Self {
        Self {
            coeffs: std::array::from_fn(|idx| *coeffs[idx]),
        }
    }
    ...
    fn from_coeffs_in_base_iter<I: Iterator<Item = F>>(mut coefs_iter: I) -> Self {
        Self {
            coeffs: std::array::from_fn(|_| coefs_iter.next().unwrap()),
        }
    }

    #[inline(always)]
    fn from_base(elem: F) -> Self {
        let coeffs = std::array::from_fn(|idx: usize| if idx == 0 { elem } else { F::ZERO });
        Self { coeffs }
    }
