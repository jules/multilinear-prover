...
pub fn adjust_to_zero_c0_var_length<const N: usize, A: Allocator + Clone>(
    trace_columns: &mut RowMajorTrace<N, A>,
...
pub fn row_major_ifft<const N: usize, const M: usize>(
    trace_columns: &mut RowMajorTraceFixedColumnsView<N, M>,
    precomputations: &[M31_2],
) {
    assert!(M >= 2);
    let trace_len = trace_columns.len();
            ...
            for thread_idx in 0..worker.num_cores {
                scope.spawn(move |_| {
                    ...
                    for stage_idx in 0..num_stages {
                        ...
                        // Also in the first stage we definately can not split our parallelism over independent problems,
...
unsafe fn fft_inner_loop_logic_scale_and_twiddle(
    ...
    ptr: PointerWrapper<M31>,
    twiddle_ptr: PointerWrapper<M31_2>,
    scales_ptr: PointerWrapper<M31_2>,
) {
    let twiddle = twiddle_ptr.0.add(twiddle_index).read();
    let idx_1 = j;
    ...
    for cycle in 0..num_unrolled_cycles {
        let last_cycle = cycle == num_unrolled_cycles - 1;
        ...
        seq!(N in 0..32 {
            let u_el_ptr = u_ptr.add(2*N).cast::<M31_2>();
            let v_el_ptr = v_ptr.add(2*N).cast::<M31_2>();
