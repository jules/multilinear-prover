// this file implements all auxilary routines required for FFT as well as basic radix_2_implementations
use std::alloc::Global;

use crate::field::m31::complex::M31_2;
...
pub struct DomainBoundLdePrecomputations<A: GoodAllocator> {
    pub bitreversed_powers: Vec<Vec<M31_2, A>>,
    pub taus: Vec<M31_2>,
    pub domain_size: usize,
    pub domain_coset_index: usize,
    pub lde_factor: usize,
    pub coset_offset: M31_2,
}
...
impl<A: GoodAllocator> DomainBoundLdePrecomputations<A> {
    pub fn new(
        ...
        assert!(tau_order <= M31_2::two_adic_group_order());

        let tau = domain_generator_for_size::<M31_2>(tau_order as u64);
        let omega = domain_generator_for_size::<M31_2>(domain_size as u64);

        assert_eq!(tau.pow(tau_order as u32), M31_2::ONE);
        assert_eq!(omega.pow(domain_size as u32), M31_2::ONE);
        ...
        let coset_generators =
            materialize_powers_serial_starting_with_one::<M31_2, Global>(tau, lde_factor);

        let ifft_merged_scale = M31_2::from_base(M31(domain_size as u32));
        ...
        for (idx, tau) in coset_generators.iter().enumerate() {
            ...
            // In case if we go from the larget domain to some other domain, we should not scale, because it's not
...
impl<A: GoodAllocator> LdePrecomputations<A> {
    pub fn new(
        ...
        assert_eq!(domain_bound_precomputations.len(), lde_factor);
